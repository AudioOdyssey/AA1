...
class User(UserMixin):
    ...
    def get(cls, user_id):
        ...
        cur = conn.cursor()
        cur.execute(
            ("SELECT `username`, `password`, `password_salt`, `user_type`, `last_login_date`, `first_name` FROM users WHERE `user_id` = %s"), (int_user_id))
        result = cur.fetchone()
        if result['username'] is None:
            return None
        result = User(result['username'], result['password'],
                      result['password_salt'], user_type=result['user_type'], last_login_date=result['last_login_date'], first_name_input=result['first_name'])
