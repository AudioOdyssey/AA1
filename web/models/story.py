    ...
    def obj_list_json(cls, story_id):
        ...
        with conn.cursor() as cur:
            ...
            for row in query_data:
                # '' : row[],
                story_dict = {'story_id' : row[0], 'story_title' : row[1], 'story_author' : row[2], 'story_synopsis' : row[3],'story_price' : row[4], 'author_paid' : row[5], 'genre' : row[6], 'length_of_story' row[7], 'number_of_locations' : row[8], 'number_of_decisions' : row[9], 'story_in_store' : row[10], 'story_verification_date' : row[11], 'name_of_verifier' : row[12], 'verification_status' :  row[13], 'story_ratings' : row[14], 'story_language_id' : row[15], 'storage_size' : row[16], 'obj_verification_status' : row[17], 'event_verification_status' : row[18], 'user_creator_id' : row[19]}
