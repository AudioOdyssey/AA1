...
class StoryDecision:
    ...
    def get(cls, story_id, location_id, decision_id):
        ...
        with conn.cursor() as cur:
            cur.execute(("SELECT * FROM `decisions` WHERE story_id = %s AND decision_id = %s AND loc_id = %s"),
                        (story_id, decision_id, location_id))
    ...
    def dec_list(cls, story_id, loc_id):
        ...
        with conn.cursor() as cur:
            ...
            for row in results:
                decs_list.append(cls(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9],
